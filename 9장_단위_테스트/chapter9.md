...
하지만, 테스트 슈트가 없으면 개발자는 자신이 수정한 코드가 제대로 도는지 확인하 수 없다. 테스트 슈트가 없으면 이쪽을 수정해도 저쪽이 안전하다는 사실을 검증하지 못한다. 그래서 결함율이 높아지기 시작한다. 결함이 많아지면 개발자는 더이상 코드를 정리하지 못한다.

이를 초래한 원인은 테스트 코드를 막 짜도 좋다고 허용안 원인이였다.
